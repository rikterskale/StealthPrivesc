function Invoke-ExecutionCheck {
    ...
    switch($Id){
        ...
        30 { foreach($p in Get-Limited @(Get-Process -ErrorAction Stop)){try{foreach($m in $p.Modules){Add-WritablePath $m.FileName "Writable loaded module candidate (PID $($p.Id))."}}catch{Set-CheckPartial 'Some process modules could not be enumerated.' -ErrorRecord $_}} }
