function Invoke-ExecutionCheck {
    ...
    if($Id -in @(11,12,14,15,16,17,18,19,20,21,31)){
        foreach($s in Get-Limited @(Get-Services)){
            ...
            switch($Id){
                11 { Add-Evidence $s.Name 'Service inventory; raw command-line arguments omitted.' @{DisplayName=$s.DisplayName;Executable=(Get-ExecutablePath $s.PathName);Account=$s.StartName;StartMode=$s.StartMode;State=$s.State;ProcessId=$s.ProcessId} }
                12 {
                ...
                15 { foreach($sub in Get-RegistryChildren $key){Add-WritablePath $sub.PSPath 'Writable immediate service subkey.' -Registry; foreach($child in Get-RegistryChildren $sub.PSPath){Add-WritablePath $child.PSPath 'Writable second-level service subkey.' -Registry}} }
                16 { Add-ExecutableAccess $s.PathName "Writable service executable or directory candidate ($($s.Name), $($s.StartName))." }
                17 {
                ...
                18 { if($s.StartName -in @('LocalSystem','NT AUTHORITY\SYSTEM')){ $dll=Read-Registry "$key\Parameters" 'ServiceDll'; if($dll.State-eq'Present'){Add-ExecutableAccess ([Environment]::ExpandEnvironmentVariables($dll.Value)) "Writable registered LocalSystem ServiceDll ($($s.Name))."} } }
    ...
    if($Id -in @(22,23,24,25,26,27)){
        foreach($task in Get-Limited @(Get-Tasks)){
            ...
            switch($Id){
                ...
                23 { foreach($a in $task.Actions){if($a.PSObject.Properties['Execute']){Add-ExecutableAccess $a.Execute "Scheduled-task executable candidate ($name, $account).";if($a.PSObject.Properties['Arguments'] -and $a.Arguments -match '(?i)(?:-file\s+|^)("[^"]+\.(?:ps1|bat|cmd|vbs)"|\S+\.(?:ps1|bat|cmd|vbs))'){Add-ExecutableAccess $Matches[1] "Scheduled-task script candidate ($name)."}}} }
    ...
    switch($Id){
        28 {
            ...
            foreach($path in @([Environment]::GetFolderPath('Startup'),[Environment]::GetFolderPath('CommonStartup'))){Add-WritablePath $path 'Writable startup directory.';foreach($file in Get-BoundedFiles @($path) -Depth 0){Add-Artifact $file.FullName 'Startup'}}
        }
        ...
        32 { foreach($app in Get-Limited @(Get-Applications)){if($app.Location){Add-WritablePath $app.Location "Writable installed application directory ($($app.Name))."}} }
            ...
            foreach($root in @('HKLM:\SOFTWARE\Classes\CLSID','HKLM:\SOFTWARE\Classes\WOW6432Node\CLSID','HKCU:\SOFTWARE\Classes\CLSID')){
                foreach($clsid in Get-Limited @(Get-RegistryChildren $root)){foreach($type in @('InprocServer32','LocalServer32')){
                    ...
                    switch($Id){
                        ...
                        38 {if(-not[IO.Path]::IsPathRooted($path)){Add-Evidence $key 'Relative COM module registration; loader search and module existence require validation.' @{Module=$path} 'Low'}}
                        39 {if([IO.Path]::IsPathRooted($path) -and -not[IO.File]::Exists($path)){Add-Evidence $key 'COM target missing or inaccessible; stale-registration candidate.' @{Module=$path} 'Low'}}
                    }
                }}
            }
        }
        ...
        41 { foreach($hive in Get-Limited @(Get-RegistryChildren 'Registry::HKEY_USERS')){Add-WritablePath ($hive.PSPath+'\Software\Microsoft\Input\Settings') 'Writable loaded-user input settings; TypingInsights-specific effects need verification.' -Registry} }
        42 {
