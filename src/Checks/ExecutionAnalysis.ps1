    ...
    switch($Id){
        19 {
            ...
            foreach($service in Get-Limited @(Get-Services|Where-Object StartName -in @('LocalSystem','NT AUTHORITY\SYSTEM'))){
                ...
                catch{Set-CheckPartial 'Some service recovery settings are inaccessible.'}
            }
        }
        22 {
        ...
        30 {
            foreach($process in Get-Limited @(Get-CimInstance Win32_Process -ErrorAction Stop)){
                ...
                catch{Set-CheckPartial 'Some processes exited or could not be inspected.'}
            }
        }
        42 {
            ...
            foreach($process in Get-Limited @(Get-CimInstance Win32_Process -ErrorAction Stop)){
                ...
                }catch{Set-CheckPartial 'Some process/thread owners or objects were inaccessible or exited.'}
            }
        }
    }
}
