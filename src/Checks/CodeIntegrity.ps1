function Invoke-CodeIntegrityCheck {
    ...
    if((Get-FilePresence $ciTool)-eq'Present'){try{$active=@((Invoke-ReadOnlyCommand $ciTool '-lp -json'|ConvertFrom-Json).Policies);foreach($policy in $active){Add-Evidence $policy.PolicyID 'Code Integrity runtime policy state.' ($policy|Select-Object PolicyID,BasePolicyID,FriendlyName,IsEnforced,IsAuthorized,IsOnDisk)}}catch{Set-CheckPartial 'CiTool runtime policy state unavailable.'}}
