function Invoke-ExtendedInventoryCheck {
    ...
    switch($Id){
        56 {
            ...
            }catch{Set-CheckPartial 'Windows Update history API unavailable.' -ErrorRecord $_}
            finally{foreach($com in @($history,$searcher,$session)){if($com){[void][Runtime.InteropServices.Marshal]::FinalReleaseComObject($com)}}}
            try{foreach($package in Get-Limited @(Get-WindowsPackage -Online -ErrorAction Stop)){Add-Evidence $package.PackageName 'Component-based servicing package.' ($package|Select-Object PackageState,ReleaseType,InstallTime)}}catch{Set-CheckPartial 'CBS package inventory requires DISM and sufficient access.' -ErrorRecord $_}
        }
        {$_-in@(58,61)} {
            ...
            foreach($driver in Get-Limited @(Get-InstalledDriverFiles)){
                ...
                }catch{Set-CheckPartial 'A driver signature could not be evaluated.' -ErrorRecord $_}
            }
        }
    }
}
